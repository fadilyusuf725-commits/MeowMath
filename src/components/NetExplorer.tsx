...
export function NetExplorer({ initialShape = 'cube', showEnrichment = true, className = '', onCheck }: NetExplorerProps) {
  ...
  useEffect(() => {
    if (shape === 'sphere') { setFoldStep(0); return }
    const nextLayout = defaultLayout(shape)
    setLayoutId(nextLayout.id); setFoldStep(0)
  }, [shape])

  return (
    <section className={`net-lab ${className}`.trim()} aria-labelledby="net-explorer-title">
      <style>{NET_LAB_STYLES}</style>
      <header className="net-lab__heading"><div><p className="net-lab__eyebrow">MIO MENGAJAKMU MENCOBA</p><h3 id="net-explorer-title" aria-label="Jaring-jaring bangun ruang">Lipat pelan-pelan</h3><p>Pilih satu bentuk, lalu ikuti langkahnya sampai bidang datar berubah menjadi bangun ruang.</p></div><span className="net-lab__cat" aria-hidden="true">🐱</span></header>
      ...
      {shape === 'sphere' || !activeLayout ? <SpherePanel /> : <section className="net-lab__fold-mode" aria-labelledby="net-fold-heading"><div className="net-lab__fold-copy"><p className="net-lab__eyebrow">LIPAT LANGKAH DEMI LANGKAH</p><h4 id="net-fold-heading">Langkah {foldStep + 1} dari {info.foldSteps.length}: {currentFoldStep?.title}</h4><p>{currentFoldStep?.description}</p><div className="net-lab__step-dots" aria-label={`Tahap lipat ${foldStep + 1} dari ${info.foldSteps.length}`}>{info.foldSteps.map((item, index) => <button type="button" key={item.title} aria-label={`Buka langkah ${index + 1}: ${item.title}`} aria-current={foldStep === index ? 'step' : undefined} className={foldStep === index ? 'is-active' : ''} onClick={() => setFoldStep(index)}>{index + 1}</button>)}</div><div className="net-lab__fold-actions"><button type="button" onClick={() => setFoldStep((current) => Math.max(0, current - 1))} disabled={foldStep === 0}>← Sebelumnya</button><button type="button" onClick={() => setFoldStep((current) => Math.min(info.foldSteps.length - 1, current + 1))} disabled={foldStep === info.foldSteps.length - 1}>Lanjut →</button></div></div><div className="net-lab__fold-visual"><div><p>Bagian yang dilipat pada langkah ini</p><NetDiagram layout={activeLayout} selectedPieceId={null} activePieceIds={currentFoldStep?.activePieces} compact /></div><NetFold3D shape={shape} step={foldStep} totalSteps={info.foldSteps.length} activePieces={currentFoldStep?.activePieces} /></div><p className="net-lab__honesty">Model 3D ini dapat diputar dengan seret atau sentuh. Setiap bidang berputar pada rusuknya; kamu juga bisa mencoba melipat jaring-jaring dari kertas.</p></section>}
