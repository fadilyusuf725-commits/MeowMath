...
export function NetExplorer({ initialShape = 'cube', showEnrichment = true, className = '', onCheck }: NetExplorerProps) {
  ...
  return (
    <section className={`net-lab ${className}`.trim()} aria-labelledby="net-explorer-title">
      ...
      <header className="net-lab__heading"><div><p className="net-lab__eyebrow">MIO MENGAJAKMU MENCOBA</p><h3 id="net-explorer-title" aria-label="Jaring-jaring bangun ruang">Buka, sentuh, lalu lipat</h3><p>Pilih satu bentuk. Sentuh bidang berwarna untuk mengenalnya, lalu tekan <b>Lipat</b> saat kamu siap melihat perubahannya.</p></div><span className="net-lab__cat" aria-hidden="true">🐱</span></header>
      <div className="net-lab__shape-tabs" role="tablist" aria-label="Pilih bentuk yang ingin dicoba">{shapeInfos.map((item) => <button type="button" role="tab" key={item.id} aria-selected={shape === item.id} className={shape === item.id ? 'is-active' : ''} onClick={() => setShape(item.id)}><span aria-hidden="true">{item.icon}</span>{item.label}</button>)}</div>
      ...
      {shape === 'sphere' || !activeLayout ? <SpherePanel /> : <>
        <div className="net-lab__mode-tabs" role="tablist" aria-label="Pilih cara melihat bentuk"><button type="button" role="tab" aria-selected={mode === 'open'} className={mode === 'open' ? 'is-active' : ''} onClick={() => setMode('open')}>1. Lihat terbuka</button><button aria-label="2. Lipat perlahan" type="button" role="tab" aria-selected={mode === 'fold'} className={mode === 'fold' ? 'is-active' : ''} onClick={() => { setMode('fold'); setFoldStep(0) }}>2. Lipat pelan-pelan</button></div>
        ...
        {mode === 'open' ? <div className="net-lab__open-grid">
          <section className="net-lab__workbench" aria-labelledby="net-workbench-heading"><div className="net-lab__section-title"><div><p className="net-lab__eyebrow">SENTUH BAGIANNYA</p><h4 id="net-workbench-heading">{activeLayout.title}</h4></div><span>{activeLayout.pieces.length} bagian</span></div><NetDiagram layout={activeLayout} selectedPieceId={selectedPieceId} onSelectPiece={(piece) => setSelectedPieceId(piece.id)} /><p className="net-lab__net-summary">{activeLayout.summary}</p><p className="net-lab__tap-note">Ketuk bidang berwarna yang ingin kamu kenali.</p><div className="net-lab__selected-piece" aria-live="polite"><b>{selectedPiece?.role ?? 'Pilih satu bidang'}</b><p>{selectedPiece ? `Bagian ini akan menjadi ${selectedPiece.role.toLowerCase()} saat bangun dilipat.` : 'Pilih satu bidang berwarna, ya.'}</p></div></section>
          <aside className="net-lab__guide"><p className="net-lab__eyebrow">COBA BERSAMA MIO</p><h4>Apa yang kamu lihat?</h4><ul>{activeLayout.checks.map((check) => <li key={check}>{check}</li>)}</ul><button aria-label="Periksa jaring-jaring" type="button" className="net-lab__check" onClick={checkNet}>Cek idemu</button><div className={`net-lab__feedback ${checked ? 'is-visible' : ''} ${activeLayout.isValid ? 'is-valid' : 'is-invalid'}`} aria-live="polite">{checked ? <><b>{activeLayout.isValid ? '✓ Ini jaring-jaring yang tepat. Susunan ini bisa menjadi bangun.' : '↻ Ini belum menjadi jaring-jaring yang tepat. Coba susunan yang lain, ya.'}</b><p>{activeLayout.feedback}</p></> : <p>Amati dulu, lalu tekan tombol ini saat ingin tahu pendapat Mio.</p>}</div></aside>
