...
export function NetExplorer({ initialShape = 'cube', showEnrichment = true, className = '', onCheck }: NetExplorerProps) {
  ...
  return (
    <section className={`net-lab ${className}`.trim()} aria-labelledby="net-explorer-title">
      ...
      <header className="net-lab__heading"><div><p className="net-lab__eyebrow">MIO MENGAJAKMU MENCOBA</p><h3 id="net-explorer-title">Buka, sentuh, lalu lipat</h3><p>Pilih satu bentuk. Sentuh bidang berwarna untuk mengenalnya, lalu tekan <b>Lipat</b> saat kamu siap melihat perubahannya.</p></div><span className="net-lab__cat" aria-hidden="true">🐱</span></header>
      ...
      {shape === 'sphere' || !activeLayout ? <SpherePanel /> : <>
        ...
        {mode === 'open' ? <div className="net-lab__open-grid">
          ...
          <aside className="net-lab__guide"><p className="net-lab__eyebrow">COBA BERSAMA MIO</p><h4>Apa yang kamu lihat?</h4><ul>{activeLayout.checks.map((check) => <li key={check}>{check}</li>)}</ul><button type="button" className="net-lab__check" onClick={checkNet}>Cek idemu</button><div className={`net-lab__feedback ${checked ? 'is-visible' : ''} ${activeLayout.isValid ? 'is-valid' : 'is-invalid'}`} aria-live="polite">{checked ? <><b>{activeLayout.isValid ? '✓ Susunan ini bisa menjadi bangun.' : '↻ Coba susunan yang lain, ya.'}</b><p>{activeLayout.feedback}</p></> : <p>Amati dulu, lalu tekan tombol ini saat ingin tahu pendapat Mio.</p>}</div></aside>
